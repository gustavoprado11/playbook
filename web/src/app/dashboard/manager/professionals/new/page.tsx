'use client';

import { useState } from 'react';
import Link from 'next/link';
import { useRouter, useSearchParams } from 'next/navigation';
import { createProfessional } from '@/app/actions/professionals';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import {
    Select, SelectContent, SelectItem, SelectTrigger, SelectValue
} from '@/components/ui/select';
import { ArrowLeft } from 'lucide-react';
import { toast } from 'sonner';

export default function NewProfessionalPage() {
    const router = useRouter();
    const searchParams = useSearchParams();
    const preselectedType = searchParams.get('type');
    const [error, setError] = useState<string | null>(null);
    const [passwordError, setPasswordError] = useState<string | null>(null);
    const [isLoading, setIsLoading] = useState(false);
    const [professionType, setProfessionType] = useState<string>(
        preselectedType === 'nutritionist' || preselectedType === 'physiotherapist' ? preselectedType : ''
    );

    async function handleSubmit(formData: FormData) {
        setPasswordError(null);
        setError(null);

        const password = formData.get('password') as string;
        const confirmPassword = formData.get('confirm_password') as string;

        if (password.length < 6) {
            setPasswordError('A senha deve ter no mínimo 6 caracteres');
            return;
        }

        if (password !== confirmPassword) {
            setPasswordError('As senhas não coincidem');
            return;
        }

        if (!professionType) {
            setError('Selecione o tipo de profissional');
            return;
        }

        formData.set('profession_type', professionType);
        setIsLoading(true);

        const result = await createProfessional(formData);

        if (result?.error) {
            setError(result.error);
            setIsLoading(false);
        } else {
            toast.success('Profissional cadastrado com sucesso');
            router.push('/dashboard/manager/team');
        }
    }

    return (
        <div className="max-w-2xl mx-auto space-y-6">
            <div className="flex items-center gap-4">
                <Link href="/dashboard/manager/team">
                    <Button variant="ghost" size="icon">
                        <ArrowLeft className="h-5 w-5" />
                    </Button>
                </Link>
                <div>
                    <h1 className="text-2xl font-bold text-zinc-900">Novo Profissional</h1>
                    <p className="mt-1 text-zinc-500">Cadastrar nutricionista ou fisioterapeuta</p>
                </div>
            </div>

            <Card>
                <CardHeader>
                    <CardTitle>Informações do Profissional</CardTitle>
                    <CardDescription>
                        Defina os dados de acesso e informações do profissional
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <form action={handleSubmit} className="space-y-4">
                        <Input
                            name="full_name"
                            label="Nome completo"
                            placeholder="Maria Silva"
                            required
                        />

                        <Input
                            name="email"
                            type="email"
                            label="E-mail"
                            placeholder="maria@email.com"
                            required
                        />

                        <div className="w-full">
                            <label className="mb-1.5 block text-sm font-medium text-zinc-700">
                                Tipo de profissional
                            </label>
                            <Select value={professionType} onValueChange={setProfessionType}>
                                <SelectTrigger>
                                    <SelectValue placeholder="Selecione o tipo" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="nutritionist">Nutricionista</SelectItem>
                                    <SelectItem value="physiotherapist">Fisioterapeuta</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>

                        <Input
                            name="password"
                            type="password"
                            label="Senha inicial"
                            placeholder="Mínimo 6 caracteres"
                            required
                            minLength={6}
                            error={passwordError && passwordError.includes('mínimo') ? passwordError : undefined}
                        />

                        <Input
                            name="confirm_password"
                            type="password"
                            label="Confirmar senha"
                            placeholder="Repita a senha"
                            required
                            minLength={6}
                            error={passwordError && passwordError.includes('coincidem') ? passwordError : undefined}
                        />

                        <Input
                            name="start_date"
                            type="date"
                            label="Data de início"
                            defaultValue={new Date().toISOString().split('T')[0]}
                        />

                        <div className="w-full">
                            <label className="mb-1.5 block text-sm font-medium text-zinc-700">
                                Observações
                            </label>
                            <textarea
                                name="notes"
                                rows={3}
                                className="flex w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm transition-colors placeholder:text-zinc-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                                placeholder="Observações opcionais..."
                            />
                        </div>

                        {error && (
                            <div className="rounded-lg bg-red-50 p-3 text-sm text-red-600">
                                {error}
                            </div>
                        )}

                        <div className="flex gap-3 pt-4">
                            <Link href="/dashboard/manager/team" className="flex-1">
                                <Button type="button" variant="outline" className="w-full">
                                    Cancelar
                                </Button>
                            </Link>
                            <Button type="submit" className="flex-1" isLoading={isLoading}>
                                Cadastrar
                            </Button>
                        </div>
                    </form>
                </CardContent>
            </Card>
        </div>
    );
}
